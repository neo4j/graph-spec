...
// library opens the native shared library exactly once and returns its handle. Opening
// is lazy (on first use), any failure is returned to whichever call first triggers it.
...
// resolveLibraryPath decides which shared library to open, in order:
//  1. the GRAPHDATAMODEL_LIB_PATH override, used verbatim when set
//  2. the embedded library, extracted to a private temp file
//
// When neither is available it returns an error.
